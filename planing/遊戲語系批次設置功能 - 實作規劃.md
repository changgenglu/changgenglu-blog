...
| v1.2 | 2026-01-22 | 實作調整：新增 Feature Seeder 權限設定 |
...
#### 權限變更 (Feature Seeder)
| Feature ID | 名稱 | 職責 | 權限類型 |
|------------|------|------|---------|
| 252 | 遊戲批次語系設置 | 控制批量修改遊戲語系的權限 | `is_ctl` (僅控端) |
...
| `database/seeders/Feature.php` | 在「遊戲列表」下新增「遊戲批次語系設置」權限節點。 |
...
| 6 | 修改 `database/seeders/Feature.php` 新增權限控制 | - |
| 7 | 整合回傳資料並進行測試 | 5, 6 |
...
| 部署前 | Seeder | 需執行 `php artisan db:seed --class=Feature` 更新權限表 |
| 部署中 | Config | 無特殊環境變數 |
