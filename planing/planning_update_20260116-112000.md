...
| `app/Http/Controllers/GameStatusController.php` | 整合控端遊戲狀態變更推播 (全域) |
| `app/Http/Controllers/AgentGameStatusController.php` | 整合管端遊戲狀態變更推播 (單一平台) |
| `app/Http/Controllers/ProviderGameController.php` | 整合控端營運平台遊戲狀態推播 (指定平台) |
...
| POST | `/api/admin/provider-games/release` | 控端指定平台遊戲上架並推播 | admin |
...
| 操作 | 允許角色 | 特殊條件 |
|-----|---------|---------|
| 控端操作 | admin | 全域廣播 (publish) 或 指定平台 (publishProvider) |
...
| 4 | 在 `ProviderGameController` 注入推播邏輯 (需 client_id) | 1 |
| 5 | 在 `PlatformController` 注入供應商推播 | 1 |
| 6 | 在 `AgentPlatformController` 注入供應商推播 | 1 |
| 7 | 在 `PlatformCountryController` 注入地區推播 | 1 |
| 8 | 執行代碼風格檢查 (`pint`) | 2-7 |
        ...
        if result.success:
            try:
                ...
                if isBroadcast:
                    ...
                    pusher.publishProvider(provider, CHANNEL, EVENT, payload)
            catch error:
