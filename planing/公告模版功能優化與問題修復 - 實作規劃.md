...
| v1.1 | 2026-01-19 18:45 | 移除 `type` 欄位的 `Rule::in` 驗證，遵循專案慣例並確認商業邏輯已涵蓋檢查 |
...
- **不包含**（依據指示排除）：
  ...
  - Validation Rules 額外的 Enum 檢查（已確認商業邏輯有包含）。
- **假設條件**：
...
| `app/Http/Controllers/AnnouncementTemplateController.php` | 1. 調整 `use` 順序與移除未使用 Import。<br>2. 優化 `createBy...` 方法中的資料回傳邏輯（移除多餘查詢）。<br>3. 補足 return 前空行。 |
...
| 欄位 | 原規則 | 新規則 | 說明 |
|-----|-------|-------|-----|
| type | `required, integer` | `required, integer` | 維持原規則，業務邏輯已檢查合法性 |
| sub_type | `integer` | `integer, min:0` | 確保子類型非負數 |
...
| 3 | 清理 Controller Import（順序調整、移除未使用） | `AnnouncementTemplateController.php` | 低 |
| 4 | 優化 Controller 建立後回傳邏輯（使用 `refresh` 或物件屬性，避免再次 DB 查詢） | `AnnouncementTemplateController.php` | 中 |
| 5 | 優化 Service `lists` 方法（加入 Eager Loading 支援避免 N+1） | `AnnouncementTemplate.php` (Service) | 中 |
| 6 | 補足 Controller 內 return 前的空行 | `AnnouncementTemplateController.php` | 低 |
...
### 4.3 錯誤處理設計
- `ValidationException`: 當輸入格式錯誤時，Laravel 自動拋出 422 錯誤。
...
| 建立模版後檢查 SQL Log | 確認無重複 `select * from announcement_templates where id = ?` | Task 4 |
| 列表查詢 (含 contents) | 確認 SQL 為 2 次查詢 (Template + Contents)，非 N+1 | Task 5 |
