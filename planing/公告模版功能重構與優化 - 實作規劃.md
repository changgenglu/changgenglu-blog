...
| v1.3 | 2026-01-21 16:35 | 範圍調整：明確使用 B 方案 (SRP 重構，不含 DRY/DI) |
...
### 1.1 背景與目標
- **需求背景**：根據 2026-01-19 的 Code Review 報告，`feature-announcement-templates` 分支存在多項技術債與潛在問題。經主管指示，將採用 **方案 B：職責分離 (SRP Refactor)** 進行重構，將 Controller 內的業務邏輯下沉至 Service 層，但**暫不進行 Backend/Agent 共用邏輯抽取 (DRY) 與依賴注入 (DI) 重構**。
- **功能目標**：
