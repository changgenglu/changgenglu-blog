...
- [Visual Studio Code 環境設置](#visual-studio-code-環境設置)
  ...
  - [快速鍵](#快速鍵)
  - [快速鍵](#快速鍵-1)
  - [檢查 coding style(phpcs)](#檢查-coding-stylephpcs)
...
## 檢查 coding style(phpcs)

coding style 設定範本

```xml
<?xml version="1.0"?>
<ruleset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" name="WordPress Core"
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#no-shorthand-php-tags
    #############################################################################
    -->
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#single-and-double-quotes
    #############################################################################
    -->
    ...
    <!-- Covers rule: There should only be one space between the path and the include/require
    keywords.
         Note: the sniff covers more language constructs than just include/require. While not in the
    handbook,
    ...
    <!-- Covers rule: It is _strongly recommended_ to avoid reserved keywords as function parameter
    names. -->
    <rule ref="Universal.NamingConventions.NoReservedKeywordParameterNames" />

    <!-- Covers rule: Class, trait, interface, enum names should use capitalized words separated by
    underscores. -->
    ...
    <!-- Rule: For files containing test classes, the file name should reflect the class name
    exactly, as per PSR4.
    ...
    <!-- Comma spacing in function calls is handled by the
    Generic.Functions.FunctionCallArgumentSpacing sniff. -->
    ...
    <!-- Covers rule: Put spaces on both sides of the opening and closing parentheses of control
    structure blocks. -->
    <rule ref="WordPress.WhiteSpace.ControlStructureSpacing" />

    <!-- Covers rule: Define a function like so: function my_function( $param1 = 'foo', $param2 =
    'bar' ) { -->
    ...
    <!-- Prevent duplicate message. This is now checked by the
    ...
    <!-- WP demands brace on same line, not on the next line. Silence errors related to "brace on
    same line". -->
    ...
    <!-- N.B.: This sniff also checks the case of (parameter/return) type declarations, not just
    type casts. -->
    ...
    <!-- Covers rule: In a switch block, there must be no space between the case condition and the
    colon. -->
    ...
    <!-- Implied through the examples: Object operators should not have whitespace around them
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#multiline-function-calls
    #############################################################################
    -->
    <!-- Covers rule: When splitting a function call over multiple lines, each parameter must be on
    a separate line. -->
    ...
    <!-- Implied through the examples: There should no space around the union or intersection type
    operators. -->
    ...
    <!-- Covers rule: When using the ::class constant for class name resolution, the class keyword
    ...
    <!-- Covered by a variety of sniffs which deal with function calls, function declarations, array
    declarations. -->

    <!-- Covers rule: There should be no spaces between the spread operator and the
    variable/function call it applies to. -->
    <rule ref="Generic.WhiteSpace.SpreadOperatorSpacingAfter" />

    <!-- Covers rule: When combining the spread operator with the reference operator (&), there
    should be no spaces between them.. -->
    <!-- Covered by the Squiz.Functions.FunctionDeclarationArgumentSpacing.SpacingAfterReference
    error code. -->
    ...
    #############################################################################
    -->
    <!-- Rule: Each part of a namespace name should consist of capitalized words separated by
    underscores.
         https://github.com/WordPress/WordPress-Coding-Standards/issues/2304 -->

    <!-- Rule: Namespace declarations should have exactly one blank line before the declaration and
    ...
    <!-- Covers rule: ... and it should be at the top of the file.
         Note: with only one namespace declaration, it not being at the top of the file would be a parse
    error. -->
    ...
    <!-- Covers rule: Explicit global namespace declaration (namespace declaration without name) are
    also not allowed. -->
    ...
    #############################################################################
    -->
    <!-- Covers rule: Import use statements should be at the top of the file and follow the
    (optional) namespace declaration. -->
    <rule ref="PSR12.Files.FileHeader.IncorrectOrder" />

    <!-- Covers rule: Import use statements should follow a specific order based on the type of the
    import:
    ...
    <!-- Spacing after "use" keyword: covered by the Generic.WhiteSpace.LanguageConstructSpacing
    sniff. -->
    ...
    <!-- Implied through the examples: Names in an import use statement should not start with a
    leading backslash. -->
    ...
    <!-- Implied through the examples: When using group use statements, there should be one
    statement
    ...
    <!-- Prevent duplicate messages - spacing after use is already covered by
    `Generic.WhiteSpace.LanguageConstructSpacing`. -->
    <rule ref="PSR12.Traits.UseDeclaration.SpaceAfterUse">
        <severity>0</severity>
    </rule>
    <!-- Prevent duplicate messages - spacing around a comma is already covered by
    ...
    #############################################################################
    -->
    <!-- Covers rule: For all constructs that allow it (properties, methods, class constants since
    PHP 7.1),
    ...
    #############################################################################
    -->
    <!-- Covers rule: When using multiple modifiers for a class declaration, the order should be as
    follows:
    ...
    <!-- Covers rule: When using multiple modifiers for a constant declaration inside
    ...
    <!-- Covers rule: When using multiple modifiers for a property declaration, the order should be
    as follows:
    ...
    <!-- Covers rule: When using multiple modifiers for a method declaration, the order should be as
    follows:
         - First the optional abstract or final modifier.
    ...
    #############################################################################
    -->
    <!-- Covers rule: When instantiating a new object instance, parenthesis must always be used,
    even when not strictly necessary. -->
    <rule ref="PSR12.Classes.ClassInstantiation" />
    <rule ref="Universal.Classes.RequireAnonClassParentheses" />

    <!-- Covers rule: There should be no space between the name of the class being instantiated and
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#use-elseif-not-else-if
    #############################################################################
    -->
    ...
    #############################################################################
    -->
    <!-- Covers rule: When doing logical comparisons involving variables, always put the variable on
    the right side
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#ternary-operator
    #############################################################################
    -->
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#error-control-operator
    #############################################################################
    -->
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#database-queries
    #############################################################################
    -->
    ...
    <!-- Covers rule: in $wpdb->prepare - %s is used for string placeholders and %d is used for
    integer
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#self-explanatory-flag-values-for-function-arguments
    #############################################################################
    -->
    ...
    <!-- Covers rule: The eval() construct is very dangerous, and is impossible to secure. ... these
    must not be used. -->
    ...
    Ref:
    https://developer.wordpress.org/coding-standards/wordpress-coding-standards/php/#regular-expressions
    #############################################################################
    -->
    ...
    <!-- There should be no empty statements, i.e. lone semi-colons or open/close tags without
    content. -->
    ...
    <!-- Implied via examples: one space after keywords in class declaration, class declaration on
    one line,
