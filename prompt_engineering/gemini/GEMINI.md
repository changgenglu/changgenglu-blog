...
### 5.5 防循環操作紀律 (Anti-Loop Disciplines)

為避免觸發系統 Loop Detection 機制，必須遵守以下紀律：

1. **絕對禁止盲猜**：修改檔案前，必須先執行 `read_file` 確認目標檔案的當前內容（包含精確的縮排與換行），禁止憑記憶或假設進行 `replace`。
2. **失敗即刻暫停**：若工具呼叫（如 `replace`）報錯，**禁止立即以微調參數的方式重試**。必須先分析報錯原因，並執行一個不同的工具（如 `list_directory` 或 `read_file`）來重新確認環境，藉此打破「連續兩次相同工具失敗」的 Loop 觸發條件。
3. **路徑絕對精確**：在 Worktree 或多目錄專案環境下，工具呼叫必須使用從根目錄開始的完整路徑，避免因相對路徑解析錯誤導致連續的 "File not found" 失敗。
