...
對與 master 分支有差異的程式碼進行全面審查，產出結構化的審查報告。

## 實用指令

```bash
# 取得與 master 分支的差異（排除 lock 檔案）
git diff origin/master...HEAD -- . ':(exclude)package-lock.json' ':(exclude)composer.lock' ':(exclude)*.lock'
...
以下項目假設已由 linter/IDE/CI 處理，不需審查：
...
依據以下類別進行審核（參考對應 Skill）：

| 類別         | 權重 | 參考 Skill                |
| ------------ | ---- | ------------------------- |
| SOLID 原則   | 25%  | `architecture-reviewer`   |
| 程式碼品質   | 20%  | -                         |
| Coding Style | 15%  | `laravel-coding-standard` |
| 功能正確性   | 15%  | -                         |
| 安全性       | 15%  | `security-auditor`        |
| 效能         | 5%   | `performance-analyst`     |
| 可測試性     | 5%   | -                         |
...
| 類型      | 規則        | 正確               | 錯誤                |
| --------- | ----------- | ------------------ | ------------------- |
| 變數      | 小駝峰      | `$userEmail`       | `$user_email`       |
| 常數      | 全大寫+底線 | `COMPANY_IP`       | `CompanyIp`         |
| 類別      | 大駝峰      | `MemberController` | `member_controller` |
| Interface | I 開頭      | `IGame`            | `GameInterface`     |
...
- 格式：`前綴_描述:變數`
- 範例：`user_profile:123`、`game_code:1:2345`
...
請嘗試將報告寫入到 GitHub 儲存庫 `changgenglu/changgenglu-blog` 的 `code-review/` 目錄中。
...
- 使用專案名稱 + 分支名稱作為檔案名稱（不含時間戳或流水號）
- 範例：專案 `eagle` + 分支 `feature-inbox` → `eagle-feature-inbox.md`
