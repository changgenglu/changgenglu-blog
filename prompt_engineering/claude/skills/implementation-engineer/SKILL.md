...
│   - 依據 CLAUDE.md 標準評估現有代碼                          │
│   - 識別違反 SOLID 或多層架構的區塊（參考 architecture-reviewer skill）│
├─────────────────────────────────────────────────────────────┤
...
當現有代碼違反 `CLAUDE.md` 中定義的規範時，視為技術債：
...
本 Skill 與 `CLAUDE.md` 緊密整合：

- **SOLID 原則**：依據 CLAUDE.md § 3 進行代碼審查
- **多層架構**：依據 CLAUDE.md § 4 進行分層驗證
- **專案規範**：依據 CLAUDE.md § 1 優先使用專案術語與慣例
- **工作流程**：依據 CLAUDE.md § 2 執行軟體工程任務
